...
async function getSystemHealth() {
    try {
        ...
        try {
            ...
            console.warn('CPU usage check failed:', err.message);
        }
        ...
        try {
            ...
            memoryUsage = parseFloat(memInfo.trim()) || 0;

            const { stdout: memMB } = await execAsync("free -m | awk 'NR==2{printf \"%s\", $3 }'");
            memoryMB = parseInt(memMB.trim()) || 0;
        } catch (err) {
            console.warn('Memory usage check failed:', err.message);
        }
        ...
        try {
            ...
            uptime = parseInt(parseFloat(uptimeInfo.trim())) || 0;
        } catch (err) {
            console.warn('Uptime check failed:', err.message);
        }
        ...
        try {
            ...
            console.warn('Error count check failed:', err.message);
        }
